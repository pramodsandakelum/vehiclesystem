function fetchBookings(userRole) {
            ...
            .then(data => {
                ...
                data.forEach(booking => {
                    ...
                    if (userRole === 1 || userRole === 2) {
                        actionButtons = `<button class="btn btn-success btn-sm" onclick="handleTripAction(${booking.bid}, 'end')" ${!isApproved ? "disabled" : ""}>End Trip</button>`;
                    } else if (userRole === 3) {
                        actionButtons = `<button class="btn btn-danger btn-sm" onclick="handleTripAction(${booking.bid}, 'cancel')" ${!isApproved ? "disabled" : ""}>Cancel Trip</button>`;
                    }
...
function handleTripAction(bid,action) {
